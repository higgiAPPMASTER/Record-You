...
import { Mic, Square, Pause, Play, Save, Loader2, RefreshCcw } from "lucide-react";
...
export default function Record() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // 2. Upload audio using raw fetch
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto h-full flex flex-col">
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <Card className="flex flex-col p-8 border-border bg-card shadow-lg relative overflow-hidden">
        ...
        <Card className={`p-8 border-border bg-card transition-opacity duration-300 ${!audioBlob && !isRecording && !isSaving ? 'opacity-50 pointer-events-none grayscale-[50%]' : 'opacity-100'}`}>
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-semibold">Track Details</h2>
            <Button onClick={handleSave} disabled={!audioBlob || isSaving || !title.trim()} className="gap-2 rounded-full px-6">
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              <Label htmlFor="title" className="text-muted-foreground uppercase text-xs tracking-wider">Title *</Label>
              <Input 
                id="title" 
            ...
            <div className="space-y-2">
              <Label htmlFor="tags" className="text-muted-foreground uppercase text-xs tracking-wider">Tags (comma separated)</Label>
              <Input 
                id="tags" 
            ...
            <div className="space-y-2 flex-1">
              <Label htmlFor="notes" className="text-muted-foreground uppercase text-xs tracking-wider">Notes & Lyrics</Label>
              <Textarea 
                id="notes" 
